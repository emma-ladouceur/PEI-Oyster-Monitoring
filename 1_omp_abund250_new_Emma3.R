...
# Emma's paths
save(lstotalmax_temp_time_sal, file = "~/Dropbox/_Projects/PEI Oysters/Model_fits/OMP/lstotalmax_temp_time_sal.Rdata")
load("~/Dropbox/_Projects/PEI Oysters/Model_fits/OMP/lstotalmax_temp_time_sal.Rdata")

# Maddy's path
#save(lstotalmax_temp_time_sal, file = "~/Data/Model_fits/OMP/lstotalmax_temp_time_sal.Rdata")
load("~/Data/Model_fits/OMP/lstotalmax_temp_time_sal.Rdata")


summary(lstotalmax_temp_time_sal)
pp_check(lstotalmax_temp_time_sal) + xlim(1,1500) 
conditional_effects(lstotalmax_temp_time_sal)
...
lstAm_sal_df <- tibble(
  salinity.m = c(lstAm_ref$sal_m_q25, lstAm_ref$sal_m_q50, lstAm_ref$sal_m_q75),
  sal_label  = factor(
...
# ============================================================
# lstAm_W) TEMP ON X — prediction grid (year-specific temp support)
# ============================================================

lstAm_W_grid <- l_total_dat %>%
  ...
  unnest(water_temp) %>%
  crossing(lstAm_sal_df) %>%
  mutate(
    water_temp.m  = water_temp - lstAm_ref$wt_mean,
    n_year.m      = n_year     - lstAm_ref$yr_mean,
    #julian_date = 0,     
...
lstAm_W_ep_mat <- posterior_epred(
  lstotalmax_temp_time_sal,
  newdata    = lstAm_W_grid,
  re_formula = NA
)

# draw pool (memory-safe)
lstAm_W_pool_size <- min(2000, nrow(lstAm_W_ep_mat))
lstAm_W_draw_ids  <- sample(seq_len(nrow(lstAm_W_ep_mat)), size = lstAm_W_pool_size)

# to long + join grid meta
lstAm_W_ep_long <- lstAm_W_ep_mat[lstAm_W_draw_ids, ] %>%
  as_tibble() %>%
  ...
  left_join(
    lstAm_W_grid %>% select(row_id, n_year, water_temp, year_group, sal_label),
    by = "row_id"
  )

# ribbons: 50% + 90% + median
lstAm_W_summ <- lstAm_W_ep_long %>%
...
lstAm_W_y_max_obs  <- max(l_total_dat$larvae_total, na.rm = TRUE)
lstAm_W_y_min_plot <- max(1, min(lstAm_W_summ$low90, na.rm = TRUE))
lstAm_W_y_max_plot <- min(lstAm_W_y_max_obs * 1.1, max(lstAm_W_summ$up90, na.rm = TRUE))

# plot
lstAm_W_fig_panel <- ggplot(
  lstAm_W_summ,
  ...
  coord_cartesian(ylim = c(lstAm_W_y_min_plot, lstAm_W_y_max_plot)) +
  labs(
...
lstAm_W_ep_mat_med <- posterior_epred(
  lstotalmax_temp_time_sal,
  newdata    = lstAm_W_grid_med,
  re_formula = NA
)

lstAm_W_pool_size_med <- min(2000, nrow(lstAm_W_ep_mat_med))
lstAm_W_draw_ids_med  <- sample(seq_len(nrow(lstAm_W_ep_mat_med)), size = lstAm_W_pool_size_med)

lstAm_W_ep_long_med <- lstAm_W_ep_mat_med[lstAm_W_draw_ids_med, ] %>%
  as_tibble() %>%
  mutate(.draw = row_number()) %>%
  pivot_longer(
    cols = - .draw,
  ...
  mutate(row_id = readr::parse_number(row_id)) %>%
  left_join(
    lstAm_W_grid_med %>% select(row_id, n_year, water_temp, year_group, sal_label),
    by = "row_id"
  )

# envelope 10–90 at each x-point (row_id), then keep draws with >=95% inside
lstAm_W_bounds80_med <- lstAm_W_ep_long_med %>%
  group_by(row_id) %>%
  summarise(
...
lstAm_W_draw_keep_med <- lstAm_W_ep_long_med %>%
  left_join(lstAm_W_bounds80_med, by = "row_id") %>%
...
lstAm_W_ep_keep_med <- lstAm_W_ep_long_med %>%
  semi_join(lstAm_W_draw_keep_med, by = ".draw")

# Sample a subset ONLY for spaghetti plotting (not for slopes/intercepts)
lstAm_W_n_keep_med <- lstAm_W_draw_keep_med %>% summarise(n = n()) %>% pull(n)
lstAm_W_n_spag_med <- min(150, lstAm_W_n_keep_med)

lstAm_W_spag_ids_med <- lstAm_W_draw_keep_med %>%
  slice_sample(n = lstAm_W_n_spag_med) %>%
  pull(.draw)

lstAm_W_ep_spag_med <- lstAm_W_ep_keep_med %>%
  filter(.draw %in% lstAm_W_spag_ids_med)

# thick lines (medians) computed from ALL kept draws (aligned)
lstAm_W_summ_med <- lstAm_W_ep_keep_med %>%
  group_by(n_year, water_temp, year_group, sal_label) %>%
  summarise(
    med   = median(epred),
    low90 = quantile(epred, 0.05),
...
lstAm_W_y_max_obs_med  <- max(l_total_dat$larvae_total, na.rm = TRUE)
lstAm_W_y_min_plot_med <- max(1, min(lstAm_W_summ_med$low90, na.rm = TRUE))
lstAm_W_y_max_plot_med <- min(lstAm_W_y_max_obs_med * 1.1, max(lstAm_W_summ_med$up90, na.rm = TRUE))

# spaghetti figure (thin filtered draws + thick medians FROM SAME kept draw set)
lstAm_W_fig_spag_med <- ggplot() +
  geom_line(
    data = lstAm_W_ep_spag_med,
    aes(x = water_temp, y = epred, group = interaction(.draw, year_group), colour = year_group),
    linewidth = 0.4,
    alpha = 0.15
  ) +
  geom_line(
    data = lstAm_W_summ_med,
    aes(x = water_temp, y = med, group = year_group, colour = year_group),
    linewidth = 0.8
  ) +
  facet_wrap(~ sal_label, nrow = 1) +
...
lstAm_temp_ref <- lstAm_ref$wt_ref

lstAm_intercept_draws <- lstAm_W_ep_keep_med %>%
  group_by(.draw, n_year, year_group) %>%
  mutate(dist_to_ref = abs(water_temp - lstAm_temp_ref)) %>%
  slice_min(order_by = dist_to_ref, n = 1, with_ties = FALSE) %>%
  ungroup()

lstAm_intercept_summ <- lstAm_intercept_draws %>%
...
lstAm_fig_intercepts <- ggplot(lstAm_intercept_summ, aes(x = n_year, y = estimate)) +
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      round(lstAm_temp_ref, 1), " °C). Thick = 50% CrI; thin = 90% CrI."
    )
  ) +
  theme_bw(base_size = 18) +
  theme(panel.grid.minor = element_blank())

lstAm_fig_intercepts


# slope: within-year temperature sensitivity on log10 scale (stable for lognormal)
lstAm_slope_draws <- lstAm_W_ep_keep_med %>%
...
lstAm_slope_summ <- lstAm_slope_draws %>%
  group_by(n_year, year_group) %>%
  summarise(
...
lstAm_fig_slopes <- ggplot(lstAm_slope_summ, aes(x = n_year, y = estimate)) +
...
lstAm_W_fig_spag_med + lstAm_fig_intercepts + lstAm_fig_slopes

# ============================================================
# lstAm_T) YEAR ON X × TEMP LEVELS @ MEDIAN SALINITY (aligned)
# ============================================================

lstAm_T_temp_df <- l_total_dat %>%
  ungroup() %>%
  summarise(
...
lstAm_T_year_seq <- seq(
  floor(min(l_total_dat$n_year, na.rm = TRUE)),
  ceiling(max(l_total_dat$n_year, na.rm = TRUE)),
  by = 1
)

lstAm_T_grid <- crossing(
  n_year = lstAm_T_year_seq,
  lstAm_T_temp_df,
  tibble(salinity.m = lstAm_ref$sal_m_q50, sal_label = factor("Median salinity"))
) %>%
  mutate(
    n_year.m      = n_year     - lstAm_ref$yr_mean,
    water_temp.m  = water_temp - lstAm_ref$wt_mean,
...
lstAm_T_ep_mat <- posterior_epred(
  lstotalmax_temp_time_sal,
  newdata    = lstAm_T_grid,
  re_formula = NA
)

lstAm_T_pool_size <- min(2000, nrow(lstAm_T_ep_mat))
lstAm_T_draw_ids  <- sample(seq_len(nrow(lstAm_T_ep_mat)), size = lstAm_T_pool_size)

lstAm_T_ep_long <- lstAm_T_ep_mat[lstAm_T_draw_ids, ] %>%
  as_tibble() %>%
  mutate(.draw = row_number()) %>%
  pivot_longer(cols = - .draw, names_to = "row_id", values_to = "epred") %>%
  mutate(row_id = readr::parse_number(row_id)) %>%
  left_join(
    lstAm_T_grid %>% select(row_id, n_year, temp_label, sal_label),
    by = "row_id"
  )

# unfiltered summary ribbons (50% + 90%)
lstAm_T_summ <- lstAm_T_ep_long %>%
...
# crop on log scale
lstAm_T_y_max_obs  <- max(l_total_dat$larvae_total, na.rm = TRUE)
lstAm_T_y_min_plot <- max(1, min(lstAm_T_summ$low90, na.rm = TRUE))
lstAm_T_y_max_plot <- min(lstAm_T_y_max_obs * 1.1, max(lstAm_T_summ$up90, na.rm = TRUE))

lstAm_T_fig_time_temp <- ggplot(
  lstAm_T_summ,
...
lstAm_T_fig_time_temp


# ============================================================
# lstAm_T) INTERCEPTS + SLOPES for COOL / MEDIAN / WARM temps
# - uses the SAME lstAm_T_ep_long object from the year-on-x workflow
...
#     and your year sequence grid already used for lstAm_T_ep_long
# ------------------------------------------------------------
# Assumes these already exist from the aligned lstAm_T workflow:
#   lstAm_T_temp_df, lstAm_T_summ, lstAm_T_ep_long, lstAm_ref, lstAm_T_year_seq

# ------------------------------------------------------------
# 1) INTERCEPTS: predicted larvae at a reference year (mean year)
#    - within each draw × temp_label, take epred at year closest to ref
# ------------------------------------------------------------
lstAm_T_ref_year <- l_total_dat %>%
  ungroup() %>%
  summarise(ref_year = mean(n_year, na.rm = TRUE)) %>%
  pull(ref_year)

lstAm_T_intercept_draws <- lstAm_T_ep_long %>%
  group_by(.draw, temp_label) %>%
  mutate(dist_to_ref_year = abs(n_year - lstAm_T_ref_year)) %>%
  slice_min(order_by = dist_to_ref_year, n = 1, with_ties = FALSE) %>%
  ungroup()

lstAm_T_intercept_summ <- lstAm_T_intercept_draws %>%
  group_by(temp_label) %>%
  summarise(
    estimate = median(epred),
    lower50  = quantile(epred, 0.25),
...
lstAm_T_fig_intercepts <- ggplot(
  lstAm_T_intercept_summ,
...
# ------------------------------------------------------------
lstAm_T_slope_draws <- lstAm_T_ep_long %>%
  mutate(log10_epred = log10(epred)) %>%
  group_by(.draw, temp_label) %>%
  summarise(
...
lstAm_T_slope_summ <- lstAm_T_slope_draws %>%
  group_by(temp_label) %>%
  summarise(
    estimate = median(slope_log10),
    lower50  = quantile(slope_log10, 0.25),
...
view(lstAm_T_slope_summ)

lstAm_T_fig_slopes <- ggplot(
  lstAm_T_slope_summ,
  aes(x = temp_label, y = estimate, colour = temp_label)
) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey40") +
...
lstAm_T_fig_time_temp + lstAm_T_fig_intercepts + lstAm_T_fig_slopes
...
abund_temp_legend <- g_legend(lstAm_T_fig_time_temp)

bottom_row_abund <- ( (lstAm_T_fig_time_temp + theme(legend.position="none") ) + lstAm_T_fig_intercepts + lstAm_T_fig_slopes)/abund_temp_legend + plot_layout(heights = c(10, 1))

top_row_abund <- ( lstAm_W_fig_spag_med + lstAm_fig_intercepts + lstAm_fig_slopes)
