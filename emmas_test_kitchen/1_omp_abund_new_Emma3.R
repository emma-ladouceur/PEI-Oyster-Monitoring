...
# Emma's paths
save(ltotalmax_temp_time_sal, file = "~/Dropbox/_Projects/PEI Oysters/Model_fits/OMP/ltotalmax_temp_time_sal.Rdata")
load("~/Dropbox/_Projects/PEI Oysters/Model_fits/OMP/ltotalmax_temp_time_sal.Rdata")

# Maddy's path
#save(ltotalmax_temp_time_sal, file = "~/Data/Model_fits/OMP/ltotalmax_temp_time_sal.Rdata")
load("~/Data/Model_fits/OMP/ltotalmax_temp_time_sal.Rdata")


summary(ltotalmax_temp_time_sal)
pp_check(ltotalmax_temp_time_sal) + xlim(1,1500) 
conditional_effects(ltotalmax_temp_time_sal)
...
ltAm_sal_df <- tibble(
  salinity.m = c(ltAm_ref$sal_m_q25, ltAm_ref$sal_m_q50, ltAm_ref$sal_m_q75),
  sal_label  = factor(
...
# ============================================================
# ltAm_W) TEMP ON X — prediction grid (year-specific temp support)
# ============================================================

ltAm_W_grid <- l_total_dat %>%
  ...
  unnest(water_temp) %>%
  crossing(ltAm_sal_df) %>%
  mutate(
    water_temp.m  = water_temp - ltAm_ref$wt_mean,
    n_year.m      = n_year     - ltAm_ref$yr_mean,
...
ltAm_W_ep_mat <- posterior_epred(
  ltotalmax_temp_time_sal,
  newdata    = ltAm_W_grid,
  re_formula = NA
)

# draw pool (memory-safe)
ltAm_W_pool_size <- min(2000, nrow(ltAm_W_ep_mat))
ltAm_W_draw_ids  <- sample(seq_len(nrow(ltAm_W_ep_mat)), size = ltAm_W_pool_size)

# to long + join grid meta
ltAm_W_ep_long <- ltAm_W_ep_mat[ltAm_W_draw_ids, ] %>%
  as_tibble() %>%
  ...
  left_join(
    ltAm_W_grid %>% select(row_id, n_year, water_temp, year_group, sal_label),
    by = "row_id"
  )

# ribbons: 50% + 90% + median
ltAm_W_summ <- ltAm_W_ep_long %>%
...
ltAm_W_y_max_obs  <- max(l_total_dat$larvae_total, na.rm = TRUE)
ltAm_W_y_min_plot <- max(1, min(ltAm_W_summ$low90, na.rm = TRUE))
ltAm_W_y_max_plot <- min(ltAm_W_y_max_obs * 1.1, max(ltAm_W_summ$up90, na.rm = TRUE))

# plot
ltAm_W_fig_panel <- ggplot(
  ltAm_W_summ,
  ...
  coord_cartesian(ylim = c(ltAm_W_y_min_plot, ltAm_W_y_max_plot)) +
  labs(
...
ltAm_W_ep_mat_med <- posterior_epred(
  ltotalmax_temp_time_sal,
  newdata    = ltAm_W_grid_med,
  re_formula = NA
)

ltAm_W_pool_size_med <- min(2000, nrow(ltAm_W_ep_mat_med))
ltAm_W_draw_ids_med  <- sample(seq_len(nrow(ltAm_W_ep_mat_med)), size = ltAm_W_pool_size_med)

ltAm_W_ep_long_med <- ltAm_W_ep_mat_med[ltAm_W_draw_ids_med, ] %>%
  as_tibble() %>%
  mutate(.draw = row_number()) %>%
  pivot_longer(
    cols = - .draw,
  ...
  mutate(row_id = readr::parse_number(row_id)) %>%
  left_join(
    ltAm_W_grid_med %>% select(row_id, n_year, water_temp, year_group, sal_label),
    by = "row_id"
  )

# envelope 10–90 at each x-point (row_id), then keep draws with >=95% inside
ltAm_W_bounds80_med <- ltAm_W_ep_long_med %>%
  group_by(row_id) %>%
  summarise(
...
ltAm_W_draw_keep_med <- ltAm_W_ep_long_med %>%
  left_join(ltAm_W_bounds80_med, by = "row_id") %>%
...
ltAm_W_ep_keep_med <- ltAm_W_ep_long_med %>%
  semi_join(ltAm_W_draw_keep_med, by = ".draw")

# Sample a subset ONLY for spaghetti plotting (not for slopes/intercepts)
ltAm_W_n_keep_med <- ltAm_W_draw_keep_med %>% summarise(n = n()) %>% pull(n)
ltAm_W_n_spag_med <- min(150, ltAm_W_n_keep_med)

ltAm_W_spag_ids_med <- ltAm_W_draw_keep_med %>%
  slice_sample(n = ltAm_W_n_spag_med) %>%
  pull(.draw)

ltAm_W_ep_spag_med <- ltAm_W_ep_keep_med %>%
  filter(.draw %in% ltAm_W_spag_ids_med)

# thick lines (medians) computed from ALL kept draws (aligned)
ltAm_W_summ_med <- ltAm_W_ep_keep_med %>%
  group_by(n_year, water_temp, year_group, sal_label) %>%
  summarise(
    med   = median(epred),
    low90 = quantile(epred, 0.05),
...
ltAm_W_y_max_obs_med  <- max(l_total_dat$larvae_total, na.rm = TRUE)
ltAm_W_y_min_plot_med <- max(1, min(ltAm_W_summ_med$low90, na.rm = TRUE))
ltAm_W_y_max_plot_med <- min(ltAm_W_y_max_obs_med * 1.1, max(ltAm_W_summ_med$up90, na.rm = TRUE))

# spaghetti figure (thin filtered draws + thick medians FROM SAME kept draw set)
ltAm_W_fig_spag_med <- ggplot() +
  geom_line(
    data = ltAm_W_ep_spag_med,
    aes(x = water_temp, y = epred, group = interaction(.draw, year_group), colour = year_group),
    linewidth = 0.4,
    alpha = 0.15
  ) +
  geom_line(
    data = ltAm_W_summ_med,
    aes(x = water_temp, y = med, group = year_group, colour = year_group),
    linewidth = 0.8
  ) +
  #facet_wrap(~ sal_label, nrow = 1) +
  ...
  coord_cartesian(ylim = c(ltAm_W_y_min_plot_med, ltAm_W_y_max_plot_med)) +
  labs(
    x = "Surface water temperature (°C)",
...
ltAm_temp_ref <- ltAm_ref$wt_ref

ltAm_intercept_draws <- ltAm_W_ep_keep_med %>%
  group_by(.draw, n_year, year_group) %>%
  mutate(dist_to_ref = abs(water_temp - ltAm_temp_ref)) %>%
  slice_min(order_by = dist_to_ref, n = 1, with_ties = FALSE) %>%
  ungroup()

ltAm_intercept_summ <- ltAm_intercept_draws %>%
...
ltAm_fig_intercepts <- ggplot(ltAm_intercept_summ, aes(x = n_year, y = estimate)) +
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      round(ltAm_temp_ref, 1), " °C). Thick = 50% CrI; thin = 90% CrI."
    )
  ) +
  theme_bw(base_size = 18) +
...
ltAm_slope_draws <- ltAm_W_ep_keep_med %>%
...
ltAm_slope_summ <- ltAm_slope_draws %>%
  group_by(n_year, year_group) %>%
  summarise(
...
ltAm_fig_slopes <- ggplot(ltAm_slope_summ, aes(x = n_year, y = estimate)) +
...
ltAm_W_fig_spag_med + ltAm_fig_intercepts + ltAm_fig_slopes

# ============================================================
# ltAm_T) YEAR ON X × TEMP LEVELS @ MEDIAN SALINITY (aligned)
# ============================================================

ltAm_T_temp_df <- l_total_dat %>%
  ungroup() %>%
  summarise(
...
ltAm_T_grid <- crossing(
  n_year = ltAm_T_year_seq,
  ltAm_T_temp_df,
  tibble(salinity.m = ltAm_ref$sal_m_q50, sal_label = factor("Median salinity"))
) %>%
  mutate(
    n_year.m      = n_year     - ltAm_ref$yr_mean,
    water_temp.m  = water_temp - ltAm_ref$wt_mean,
...
ltAm_T_ep_mat <- posterior_epred(
  ltotalmax_temp_time_sal,
  newdata    = ltAm_T_grid,
  re_formula = NA
)

ltAm_T_pool_size <- min(2000, nrow(ltAm_T_ep_mat))
ltAm_T_draw_ids  <- sample(seq_len(nrow(ltAm_T_ep_mat)), size = ltAm_T_pool_size)

ltAm_T_ep_long <- ltAm_T_ep_mat[ltAm_T_draw_ids, ] %>%
  as_tibble() %>%
  mutate(.draw = row_number()) %>%
  pivot_longer(cols = - .draw, names_to = "row_id", values_to = "epred") %>%
  mutate(row_id = readr::parse_number(row_id)) %>%
  left_join(
    ltAm_T_grid %>% select(row_id, n_year, temp_label, sal_label),
    by = "row_id"
  )

# unfiltered summary ribbons (50% + 90%)
ltAm_T_summ <- ltAm_T_ep_long %>%
...
# crop on log scale
ltAm_T_y_max_obs  <- max(l_total_dat$larvae_total, na.rm = TRUE)
ltAm_T_y_min_plot <- max(1, min(ltAm_T_summ$low90, na.rm = TRUE))
ltAm_T_y_max_plot <- min(ltAm_T_y_max_obs * 1.1, max(ltAm_T_summ$up90, na.rm = TRUE))

ltAm_T_fig_time_temp <- ggplot(
  ltAm_T_summ,
 ...
 #scale_fill_viridis_d(option = "viridis") +
  ...
  coord_cartesian(ylim = c(ltAm_T_y_min_plot, ltAm_T_y_max_plot)) +
  labs(
    x = "Monitoring year",
    y = "Max oyster larvae",
...
ltAm_T_fig_time_temp


# ============================================================
# ltAm_T) INTERCEPTS + SLOPES for COOL / MEDIAN / WARM temps
# - uses the SAME ltAm_T_ep_long object from the year-on-x workflow
...
#     and your year sequence grid already used for ltAm_T_ep_long
# ------------------------------------------------------------
# Assumes these already exist from the aligned ltAm_T workflow:
#   ltAm_T_temp_df, ltAm_T_summ, ltAm_T_ep_long, ltAm_ref, ltAm_T_year_seq

# ------------------------------------------------------------
# 1) INTERCEPTS: predicted larvae at a reference year (mean year)
#    - within each draw × temp_label, take epred at year closest to ref
# ------------------------------------------------------------
ltAm_T_ref_year <- l_total_dat %>%
  ungroup() %>%
  summarise(ref_year = mean(n_year, na.rm = TRUE)) %>%
  pull(ref_year)

ltAm_T_intercept_draws <- ltAm_T_ep_long %>%
  group_by(.draw, temp_label) %>%
  mutate(dist_to_ref_year = abs(n_year - ltAm_T_ref_year)) %>%
  slice_min(order_by = dist_to_ref_year, n = 1, with_ties = FALSE) %>%
  ungroup()

ltAm_T_intercept_summ <- ltAm_T_intercept_draws %>%
  group_by(temp_label) %>%
  summarise(
    estimate = median(epred),
    lower50  = quantile(epred, 0.25),
...
ltAm_T_fig_intercepts <- ggplot(
  ltAm_T_intercept_summ,
...
# ------------------------------------------------------------
ltAm_T_slope_draws <- ltAm_T_ep_long %>%
  mutate(log10_epred = log10(epred)) %>%
  group_by(.draw, temp_label) %>%
  summarise(
...
ltAm_T_slope_summ <- ltAm_T_slope_draws %>%
  group_by(temp_label) %>%
  summarise(
    estimate = median(slope_log10),
    lower50  = quantile(slope_log10, 0.25),
...
view(ltAm_T_slope_summ)

ltAm_T_fig_slopes <- ggplot(
  ltAm_T_slope_summ,
  aes(x = temp_label, y = estimate, colour = temp_label)
) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey40") +
...
# Optional: combine with your year-on-x ribbon plot (patchwork)
 ltAm_T_fig_time_temp + ltAm_T_fig_intercepts + ltAm_T_fig_slopes
 ...
 abund_temp_legend <- g_legend(ltAm_T_fig_time_temp)

bottom_row_abund <- ( (ltAm_T_fig_time_temp + theme(legend.position="none") ) + ltAm_T_fig_intercepts + ltAm_T_fig_slopes)/abund_temp_legend + plot_layout(heights = c(10, 1))

top_row_abund <- ( ltAm_W_fig_spag_med + ltAm_fig_intercepts + ltAm_fig_slopes)
