...
la_m_fig_intercepts <- ggplot(la_m_intercept_summ, aes(x = n_year, y = estimate)) +
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      round(la_m_temp_ref, 1), " °C). Thick = 50% CrI; thin = 90% CrI."
    )
  ) +
  theme_bw(base_size = 18) +
...
la_m_fig_slopes <- ggplot(la_m_slope_summ, aes(x = n_year, y = estimate)) +
...
# Optional: combine with your year-on-x ribbon plot (patchwork)
 ...
 abund_temp_legend <- g_legend(la_m_T_fig_time_temp)

bottom_row_abund <- ( (la_m_T_fig_time_temp + theme(legend.position="none") ) + la_m_T_fig_intercepts + la_m_T_fig_slopes)/abund_temp_legend + plot_layout(heights = c(10, 1))

top_row_abund <- ( la_m_fig_med + la_m_fig_intercepts + la_m_fig_slopes)
